...
async function configureKey(force = false) {
    ...
    if (current && !force)
        return current;
    const key = await password({ message: current ? 'Nova chave da OpenAI' : 'Chave da OpenAI', mask: '*', validate: (v) => v.trim().startsWith('sk-') || 'Informe uma chave OpenAI valida.' });
...
async function collectSong() {
    ...
    if (mode === 'uta') {
        const id = await input({ message: 'ID ou URL do Uta-Net', default: '386588', validate: (v) => v.trim().length > 0 || 'Informe o ID ou URL.' });
        return fetchUtaNetSong(id);
    }
    const title = await input({ message: 'Titulo da musica', validate: (v) => v.trim().length > 0 || 'Informe o titulo.' });
...
async function main() {
    ...
    if (await confirm({ message: 'Deseja tentar atualizar a CLI agora?', default: false }))
        updateCli();
}
main().catch(error => {
